...
lblTitle=Label(Tops, text="       BMI Calculator Program  ",padx=16,pady=16,bd=16,
                fg="#000000", font=('arial', 24,'bold'),
                bg="light green", relief = 'raise', width = 36, height = 0)
lblTitle.pack()
###########################Label and Text###############################################################################
myLabel1 = Label(Tops, text="Enter your name:")  # .grid(row=0, column = 0)#creating a label widget
myTextl1 = Entry(Tops, bd=5)  # .grid(row=0, column = 1)

myLabel2 = Label(Tops, text="Enter your age")  # .grid(row=1, column = 0)
myTextl2 = Entry(Tops, bd=5)  # .grid(row=1, column = 1)
...
########################CHECK BOX Unit######################################################################################
myUnit1 = Label(Tops, text="Units:").pack()
Checkbutton(Tops, text='Metric').pack()
Checkbutton(Tops, text='Imperial').pack()
#########################SCALE OF WEIGHT################################################################################
...
txtheight = Entry(f1b, textvariable = var2, font=('arial', 16, 'bold'),bd=16, width=12, justify='center')
txtheight.grid(row = 1,column=0)
##############RESULT OUTPUT#############################################################################################
lblBMIResult = Label(f1b, padx =16, pady=16, bd=5,
                     fg="#000000", font=('arial',30),
                     bg="light green", relief ='raise', width = 42, height = 1)
...
####################BUTTON##############################################################################################
btnBMI = Button(f2, text="Enter to \ncheck your \nBMI",padx=8,pady=8,bd=12, width = 10,
